      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        {showLinks&&(
          <div style={{ position:'fixed', inset:0, zIndex:500, background:'#fff', display:'flex', flexDirection:'column' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', background:'#1a5c52', flexShrink:0 }}>
              <div style={{ fontFamily:'Geist, sans-serif', fontWeight:700, fontSize:15, color:'#fff' }}>🔗 Istanbul Links</div>
              <button onClick={()=>setShowLinks(false)} style={{ background:'rgba(255,255,255,0.15)', border:'none', color:'#fff', width:32, height:32, borderRadius:'50%', fontSize:16, cursor:'pointer' }}>✕</button>
            </div>
            <iframe src="/links.html" style={{ flex:1, border:'none', width:'100%' }} title="Istanbul Links" />
          </div>
        )}
        {showItinerary&&(
          <div style={{ position:'fixed', inset:0, zIndex:500, background:'#fff', display:'flex', flexDirection:'column' }}>
