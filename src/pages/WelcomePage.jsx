...
// Check for join token in URL on load
const JOIN_TOKEN = (() => {
  try {
...
export function WelcomePage({ onSignIn }) {
  ...
  const [step, setStep] = useState(JOIN_TOKEN ? 'invite' : 'home')
  ...
  if (step === 'invite') {
    const waMsg = encodeURIComponent(`Hi Umer! I just clicked the Wanderlog invite link and I'm about to sign in 👋\n\nPlease approve me: https://wanderlog-one.vercel.app?approve=true`)
    return (
      <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'32px 24px', fontFamily:'Geist, sans-serif', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:2, maxWidth:320, width:'100%', textAlign:'center' }}>
          ...
          <div style={{ fontSize:14, color:'#888', lineHeight:1.6, marginBottom:32, animation:'fadeUp 0.5s ease 0.2s both' }}>Umer has invited you to join the family's private travel journal.</div>

          <div style={{ background:'rgba(255,255,255,0.9)', borderRadius:20, padding:'20px', marginBottom:12, backdropFilter:'blur(8px)', boxShadow:'0 4px 20px rgba(0,0,0,0.08)', animation:'fadeUp 0.5s ease 0.3s both' }}>
            <div style={{ fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#aaa', marginBottom:12 }}>Step 1 — Let Umer know</div>
            <a href={`https://wa.me/${ADMIN_WHATSAPP}?text=${waMsg}`} target="_blank" rel="noreferrer"
              style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, background:'#25D366', color:'#fff', borderRadius:12, padding:'14px', fontSize:15, fontWeight:700, textDecoration:'none' }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.9)', borderRadius:20, padding:'20px', backdropFilter:'blur(8px)', boxShadow:'0 4px 20px rgba(0,0,0,0.08)', animation:'fadeUp 0.5s ease 0.4s both' }}>
            <div style={{ fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#aaa', marginBottom:12 }}>Step 2 — Sign in</div>
      ...
      <div style={{ position:'relative', zIndex:2, maxWidth:320, width:'100%', textAlign:'center' }}>
        ...
        <div style={{ marginTop:8, animation:'fadeUp 0.5s ease 0.5s both' }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.85)', borderRadius:16, padding:'16px', backdropFilter:'blur(8px)', boxShadow:'0 4px 20px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ fontSize:11, color:'#bbb', marginTop:6, textAlign:'left' }}>You'll receive an SMS with a 6-digit code</div>
          </div>
        </div>
